...
//    private final CameraDetected cameraDetected;
    private final Activity cameraDetected;
    ...
    public PermissionHandler(ShowMessage showMessage, Activity cameraDetected) {
    ...
    public void doForStoragePermission(){
        doPermission(PERMISSIONS_FOR_STORAGE_DISCOVERY);
    }
    /**
    ...
    private void doPermission(String[] permissions){
        for (String permission : permissions) {
            if ( ! checkPermission(permission) ) {
                requestPermission(permission);
            }
...
//        ThermalLog.d(TAG,"requestPermission(), permission:"+permission);
        boolean permissionRationale = ActivityCompat.shouldShowRequestPermissionRationale(cameraDetected, permission);
        if (permissionRationale) {
            ...
            ActivityCompat.requestPermissions(cameraDetected, permissions, requestCode);
        }
    }
...
//        ThermalLog.d(TAG,"checkPermission(), permission: "+permission);
        int checkPermission = cameraDetected.checkPermission(permission, Process.myPid(), Process.myUid());
